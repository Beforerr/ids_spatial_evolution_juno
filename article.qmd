...
$$
B_m(t) = B_{m0} \tanh\left(\frac{t - t_0}{\Delta T}\right) + c
$${#eq-fit}

where $B_{m0}$ is the amplitude of the magnetic field change in the maximum variance direction, $t_0$ is the time of discontinuity, $\Delta T$ is the transition time, and $c$ is the offset of the magnetic field change (in the case of an ideal discontinuity with perfect rotational symmetry, $c$ should theoretically be zero. However, in the context of data processing, incorporating $c$ can enhance the fitting accuracy).
